    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        case UICollectionView.elementKindSectionHeader:
            guard let header = collectionView.dequeueReusableSupplementaryView(ofKind: kind, 
                                                                               ...
                                                                               for: indexPath) as? RestaurantPhotoHeaderView else { return UICollectionReusableView() }
...
extension RestaurantDetailPhotoViewController: UICollectionViewDataSource {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return viewModel?.restaurantReviewImages.isEmpty == true ? 1 : viewModel?.restaurantReviewImages.count ?? 0
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        
        if viewModel?.restaurantReviewImages.isEmpty == true {
            guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "emptyCell",
                                                                for: indexPath) as? RestaurantEmptyPhotoCell else { return UICollectionViewCell() }
            cell.setupData(comment: "아직 등록된 사진이 없어요")
            return cell
        } else {
            guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "reviewPhotoCell", 
                                                                for: indexPath) as? RestaurantReviewPhoto2Cell else { return UICollectionViewCell() }
