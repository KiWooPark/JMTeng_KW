...
extension RestaurantDetailReviewViewController: UICollectionViewDataSource {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return viewModel?.restaurantData?.reviews.isEmpty == true ? 1 : viewModel?.restaurantData?.reviews.count ?? 0
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if viewModel?.restaurantData?.reviews.isEmpty == true {
            guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "emptyCell", 
                                                                for: indexPath) as? RestaurantEmptyReviewCell else { return UICollectionViewCell() }
            cell.setupData(comment: "아직 등록된 후기가 없어요")
            return cell
            
        } else {
            guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "reviewCell", 
                                                                for: indexPath) as? RestaurantReview2Cell else { return UICollectionViewCell() }
