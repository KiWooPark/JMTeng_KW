...
class RestaurantDetailViewController: UIViewController, KeyboardEvent {
    ...
    func didUpdateRestaurant(_ notification: Notification) {
        if let data = notification.userInfo?["restaurantData"] as? EditRestaurantModel {
            self.viewModel?.updateRestaurantInfo(model: data)
            self.viewModel?.didUpdateRestaurantSeg?()
            setupData()
        }
    }
    ...
    func fetchRestaurantData() {
        ...
        Task {
            do {
                ...
                let userInfo = UserDefaultManager.userInfo
                
                DispatchQueue.main.async {
                    
                    if userInfo?.id == self.viewModel?.restaurantData?.userId ?? 0 {
                        self.setCustomNavigationMoreButton()
                    }
