...
struct ReadRestaurantsAPI {
    ...
    static func fetchOutBoundRestaurantsAsync(request: OutBoundRestaurantsRequest) async throws -> OutBoundRestaurantsResponse {
        ...
        do {
            let response = await AF.request(ReadRestaurantsTarget.outBoundRestaurants(request), interceptor: DefaultRequestInterceptor())
                .validate()
            ...
            case 200:
                guard let result = response.value else {
                    throw RestaurantError.noOtherGroupRestaurantsAvailable
                }
                return result
            default:
                throw RestaurantError.unknownError
            }
        } catch {
            throw error
        }
    ...
    static func searchRestaurantsAsync(request: SearchRestaurantsRequest) async throws -> SearchRestaurantsResponse {
        do {
            let response = await AF.request(ReadRestaurantsTarget.searchRestaurants(request), interceptor: DefaultRequestInterceptor())
                .validate()
            ...
            case 200:
                guard let result = response.value else {
                    throw RestaurantError.noRestaurantsAvailable
                }
