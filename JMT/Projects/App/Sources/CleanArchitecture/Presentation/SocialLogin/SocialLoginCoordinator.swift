...
protocol SocialLoginCoordinator: Coordinator {
    ...
    func showGoogleLoginViewController(completion: @escaping (Result<String, NetworkError>) -> Void)
    func showAppleLoginViewController()
    
    var onAppleLoginSuccess: ((Result<String, NetworkError>) -> Void)? { get set }
}
...
class DefaultSocialLoginCoordinator: NSObject, SocialLoginCoordinator {
    ...
    // 애플 로그인 콜백
    var onAppleLoginSuccess: ((Result<String, NetworkError>) -> Void)?
    
    func start() {
        guard let initialViewController = SocialLoginViewController.instantiateFromStoryboard(storyboardName: "Login") as? SocialLoginViewController else { return }
    ...
    func logout() {
        ...
        if let window = UIApplication.shared.windows.filter({ $0.isKeyWindow }).first {
            ...
            UIView.transition(with: window, duration: 0.5, options: .transitionCrossDissolve, animations: {}) { _ in
                // 루트뷰 교체 후 작업 할 것
            }
        }
    }
    ...
    func showNicknameViewController() {
        ...
        if let nicknameCoordinator = getChildCoordinator(.nickname) as? NicknameCoordinator {
...
extension DefaultSocialLoginCoordinator: ASAuthorizationControllerDelegate, ASAuthorizationControllerPresentationContextProviding {
    ...
    func authorizationController(controller: ASAuthorizationController, didCompleteWithAuthorization authorization: ASAuthorization) {
        ...
        case let appleIDCredential as ASAuthorizationAppleIDCredential:
            ...
            {
                onAppleLoginSuccess?(.success(identifyTokenString))
            }
